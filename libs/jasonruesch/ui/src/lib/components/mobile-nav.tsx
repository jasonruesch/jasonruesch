    ...
    <nav className={twMerge('space-y-2', className)}>
      {navigation.map((page) => (
        <PageNavLink
          ...
          className={({ isActive }) =>
            twJoin(
              'px-safe-offset-4 block py-2 font-semibold leading-7 text-neutral-900 dark:text-neutral-50',
      ...
      {authenticated ? (
        <button
          type="button"
          className="px-safe-offset-4 block w-full cursor-pointer py-2 text-left font-semibold leading-7 text-neutral-900 hover:bg-neutral-300 focus-visible:bg-neutral-300 dark:text-neutral-50 dark:hover:bg-neutral-700 dark:focus-visible:bg-neutral-700"
          onClick={logout}
        >
          Sign Out
        </button>
